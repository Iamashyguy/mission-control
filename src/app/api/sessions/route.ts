...
async function getSessionMessages(sessionId: string): Promise<NextResponse> {
  ...
  // Search all agent session directories
  ...
  try {
    ...
    for (const agent of agents) {
      const candidate = join(agentsDir, agent, 'sessions', `${sessionId}.jsonl`);
    ...
    filePath = join(agentsDir, 'main', 'sessions', `${sessionId}.jsonl`);
  }
